...
const columns = [
    ...
    { field: 'name', headerName: 'Заказчик', width: 230 },
    {
        field: 'actions',
        type: 'actions',
        width: 50,
        ...
        getActions: (params) => [
            <GridActionsCellItem
                icon={<VisibilityIcon />}
                label="Открыть"
                onClick={() => location.assign(`/orders/${params.id}`)}
            />,
        ],
    },
];
...
function EditToolbar(props) {
    ...
    {
        fetch(`http://localhost:3000/orders/api/`, {
            cache: "no-store",
            method: 'DELETE',
    ...
    return (
        <GridToolbarContainer>
            <Button color="primary" startIcon={<AddIcon />} href="/orders/new">
                Добавить заказ
            </Button>
